...
import { BlockTypeEnum } from "../model/nom/accountBlock.js";
...
export function getTxHash(transaction) {
...
function getTxSignature(keyPair, transaction) {
    return keyPair.sign(transaction.hash.getBytes());
}
function getPoWData(transaction) {
    return Hash.digest(Buffer.concat([
        transaction.address.getBytes(),
        transaction.previousHash.getBytes()
    ]));
}
async function autofillTxParameters(zenonInstance, accountBlockTemplate) {
...
async function checkAndSetFields(zenonInstance, transaction, currentKeyPair) {
    ...
    if (isReceiveBlock(transaction.blockType)) {
        ...
        if (sendBlock.toAddress.toString() !== transaction.address.toString()) {
            throw new ZnnBlockUtilitiesException(`Send block toAddress (${sendBlock.toAddress}) does not match transaction address (${transaction.address})`);
        }
...
async function setDifficulty(zenonInstance, transaction) {
    const powParam = new GetRequiredPowParam(transaction.address, transaction.blockType, transaction.toAddress, transaction.data);
...
function setHashAndSignature(transaction, currentKeyPair) {
    transaction.hash = getTxHash(transaction);
    transaction.signature = getTxSignature(currentKeyPair, transaction);
    return transaction;
}
export async function send(zenonInstance, transaction, currentKeyPair) {
